...
tags:
  ...
  - report-diagram
metadata: {"homepage":"https://github.com/xiaopixuan/drawio-diagram-skill","compatibility":"Requires draw.io desktop app CLI on PATH (macOS/Linux/Windows). Self-check step requires a vision-enabled model; gracefully skipped if unavailable. Optional auto-layout needs Graphviz (dot).","platforms":["macos","linux","windows"],"openclaw":{"requires":{"anyBins":["draw.io","drawio"]},"emoji":"📐","os":["darwin","linux","win32"],"install":[{"id":"brew-drawio","kind":"brew","formula":"drawio","bins":["drawio"],"label":"Install draw.io via Homebrew","os":["darwin"]},{"id":"brew-graphviz","kind":"brew","formula":"graphviz","bins":["dot"],"label":"Install Graphviz for optional autolayout.py","os":["darwin"],"optional":true}]},"hermes":{"tags":["drawio","diagram","flowchart","architecture","visualization","uml"],"category":"design","requires_tools":["drawio","draw.io"],"related_skills":["mermaid","excalidraw","plantuml"]},"author":"xiaopixuan","version":"1.34.0"}
---
...
- Manual install: download draw.io Desktop from <https://github.com/jgraph/drawio-desktop/releases>, then install the `.dmg`, `.exe`, `.deb`, or `.rpm` for the user's OS.
...
1. `~/.drawio-skill/styles/<name>.json` — user presets (survive `git pull`; legacy compatibility path intentionally retained)
...
- `--layout <preset|json>` — **CLI ≥ v30 only** — Post-generate layout pass on XML input. Two modes: **(a) ELK** presets (`verticalFlow`, `horizontalFlow`, `verticalTree`, `horizontalTree`, `radialTree`, `organic`, or a custom ELK JSON array) re-place nodes + route edges; alternative to `autolayout.py` when Graphviz is missing. **(b) `libavoid`** keeps every vertex exactly where you placed it and only reroutes the connectors orthogonally around the shapes (obstacle-avoiding routing) — use it on architecture/network/deployment diagrams to keep your deliberate layout while cleaning up edge crossings. Never combine with Mermaid-converted files (already laid out). On ≤ 29 this flag breaks argument parsing — don't emit it. See `references/mermaid-authoring.md`
...
| Scenario | Behavior |
| ---------- | ---------- |
| draw.io CLI missing, Python available | Tell the user they do not have draw.io Desktop or the `drawio` command is not available; offer Agent install command vs manual install; then use browser fallback (diagrams.net URL) if they want to continue without installing |
| draw.io CLI missing, Python missing | Tell the user they do not have draw.io Desktop or the `drawio` command is not available; offer Agent install command vs manual install; then generate `.drawio` XML only and instruct user to open in draw.io desktop or diagrams.net manually |
...
# WSL2: the CLI is the Windows desktop exe, reached via /mnt/c (note the space)
  ...
  echo "你现在没有安装 draw.io Desktop，或当前环境找不到 drawio 命令。可以让 Agent 执行：brew install --cask drawio；也可以手动安装：https://github.com/jgraph/drawio-desktop/releases"
fi
```
